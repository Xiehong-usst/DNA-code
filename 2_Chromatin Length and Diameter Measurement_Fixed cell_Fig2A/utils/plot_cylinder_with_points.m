function plot_cylinder_with_points(A, B, radius, points, color1)
    ...
    % Plot
    surf(X, Y, Z, 'FaceAlpha', 0.3, 'FaceColor', color1, 'EdgeColor', 'none'); % Cylinder with transparency
    hold on;
    scatter3(points(:, 1), points(:, 2), points(:, 3), 'filled', 'MarkerFaceColor',color1); % Scatter points
    xlabel('X');
    ylabel('Y');
    zlabel('Z');
    axis equal;
