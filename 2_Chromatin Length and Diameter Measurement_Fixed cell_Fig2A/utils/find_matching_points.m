        ...
        for perm_idx = 1:size(permutations, 1)
            for sign_idx = 1:size(signs, 1)
                ...
                if match
                    ...
                    for dim = 1:3
                        if sign_vals(dim) == -1
                            flip_desc{end+1} = [axis_names{perm(dim)} 'axis flipped'];
                        end
                    end
                    
                    
                    disp('================================================================');
                    disp('Match found successfully! Transformation steps:');
                    disp('1. Axis permutation:');
                    fprintf('    • %s\n', perm_desc{:});
                    
                    if ~isempty(flip_desc)
                        disp('2. Axis inversion:');
                        fprintf('    • %s\n', flip_desc{:});
                    else
                       disp('2. Axis inversion: None');
                    end
                    
                   disp('3. Translation:');
                    fprintf('    • Xdirection: %.4f\n', T(1));
                    fprintf('    • Ydirection: %.4f\n', T(2));
                    fprintf('    • Zdirection: %.4f\n', T(3));
                    
                    disp('4.  Matched point IDs：');
                    fprintf('    ');
                    for i = 1:8
                        fprintf('%d ', ids(i));  
                    end
                    ...
                    disp('================================================================');
                    disp(['Complete transformation formula:' ...
                        'New coordinates = [Original' axis_names{perm(1)} ', Original' axis_names{perm(2)} ', Original' axis_names{perm(3)} ']']);
                    if sign_vals(1) == -1
                        fprintf('                × [-1, ');
                    else
                        fprintf('                × [1, ');
...
error('No matching point set found.');
end