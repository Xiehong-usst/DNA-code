function test_findNucleosome(filename, data1)
    ...
    for zu = 1:numel(nz)
        ...
        bad_point_tag = 0;
        if sum(inside_ID1) > sum(inside_ID2) + 5
            bad_point_tag = 1;
        end
        ...
        if overlap_tag == 0 && bad_point_tag == 0
            ...
            [X_cylinder, Y_cylinder, Z_cylinder] = plot_cylinder_with_points(P1, P2, bestR, rand(1,3)*0.7);
            scatter3(data3(:,1),data3(:,2),data3(:,3),50,'r','filled');
        end
        title({filename,num2str(2 * bestR)});
    end
    save_path = sprintf('./results/live/params/%s.mat', filename);
    save(save_path, 'data3_list', 'bestR_list', 'data_new1', 'X_cylinder', 'Y_cylinder', 'Z_cylinder');
