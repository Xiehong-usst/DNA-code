...
if ~exist(fullfile(pwd, 'saved_figures'), 'dir')
    mkdir('saved_figures'); 
end

if ~exist(fullfile(pwd, 'saved_variables'), 'dir')
...
for idx = 1:length(file_list)
    ...
    figure;
    scatter3(data1(:,1), data1(:,2), data1(:,3), 5, [0 0 0], 'filled');
    hold on;
    ...
    fig_name = fullfile('.\', [file_name(1:end-4) '_scatter_clusters.fig']);
    savefig(fig_name);
